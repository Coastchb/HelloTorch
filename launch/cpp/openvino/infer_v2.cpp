...
async_simple::coro::Lazy<void> tts_http_server(std::map<std::string, int>& char2id, ov::CompiledModel& model) {
    int max_thread_num = std::thread::hardware_concurrency();
    coro_http_server server(max_thread_num, 8080);
    std::cout << "max_thread_num:" << max_thread_num << std::endl;       

    server.set_http_handler<GET, POST>("/TTS", [&char2id,&model](coro_http_request& req, coro_http_response& res) {
        std::string input = "Hello,world!";
        auto req_queris = req.get_queries();
        for (auto &[k, v] : req.get_queries()) {
        ...
        tts(char2id, model, input, wav_filename);
...
int main(int argc, char* argv[]) {
    try {
        ...
        async_simple::coro::syncAwait(tts_http_server(char2id, compiled_model));
